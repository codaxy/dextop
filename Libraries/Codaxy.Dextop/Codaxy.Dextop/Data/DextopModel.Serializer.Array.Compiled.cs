...
namespace Codaxy.Dextop.Data
{
	...
	/// </summary>
    public class DextopModelDynamicArraySerializer : IDextopModelSerializer
    {
        ...
        public DextopModelDynamicArraySerializer(DextopModelTypeMeta meta)
        {
            Meta = meta;
            ...
            foreach (var field in meta.Fields)
            {
                ...
                {
                    var valueProvider = new Newtonsoft.Json.Serialization.DynamicValueProvider(property);
                    if (property.CanRead)
                        getter = (target) => { return valueProvider.GetValue(target); };
                    if (property.CanWrite)
                        setter = (target, value) => { valueProvider.SetValue(target, ChangeType(value, property.PropertyType)); };
                    Getters.Add(getter);
                    Setters.Add(setter);
                }
            }            
